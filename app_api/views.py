...
def getDefaultEmailTemplate(request):
    ...
    try:
        ...
        if not branding or not email_temp:

            acert_url = f"{getConfig()['DOMAIN']['acert']}/api/get-default-email"
            acert_res = requests.post(acert_url, timeout=10)
            acert_data = acert_res.json()

            if acert_data.get("statusCode") != 0:
                return JsonResponse(acert_data)

            acert_email = acert_data["data"]         
            acert_branding = acert_email.get("branding", {})
            default_socials = {
                "Linkedin": "#",
                "Youtube": "#"
            }

            for platform, url in default_socials.items():
            ...
            email_body = acert_email["email_body"].replace("[footer_div]", footer_div_html)


            response["data"] = {
                "id": acert_email["id"],
                "template_name": acert_email["template_name"],
                "paper_type": acert_email["paper_type"],
                "email_subject": acert_email["email_subject"],
                "email_body": email_body,
                "template_heading": acert_email["template_heading"],
                "sender_email": acert_email["sender_email"],
                "sender_label": acert_email["sender_label"],
                "email_attachment": acert_email.get("email_attachment", ""),
                "email_attachment_name": acert_email.get("email_attachment_name", ""),

                "branding": {
                    "content": acert_branding["content"],
                    "logourl": acert_branding["logourl"],
                    "social_links": {}
                },
