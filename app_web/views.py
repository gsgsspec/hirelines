...
from app_api.models import Credits, User, Role, JobDesc, CallSchedule, Candidate, Company, Branding, Profile,Source,ProfileExperience,ProfileSkills,Lookupmaster, ProfileActivity,jdlibrary,Tag
...
def resume_analysisPage(request,pid):
    if not request.user.is_active and not request.user.is_staff:
        return user_not_active(request, after_login_redirect_to=str(request.META["PATH_INFO"]))
    ...
    try:
        ...
        profile_details=getResume_AnalysisDetailsService(pid)
        
        return render(request, "portal_index.html", {"template_name": 'resume_analysis.html','menuItemList': menuItemList,'profile_details':profile_details})
