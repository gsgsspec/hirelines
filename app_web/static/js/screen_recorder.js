...
// const socket = new WebSocket('ws://localhost:8000/ws/screen-recording/');
...
// Set a variable to hold the list of initial audio output devices
let previousAudioOutputs = [];

// Function to get the current list of audio output devices
...
// Function to check for changes in audio output devices
async function checkForAudioOutputChanges() {
    const currentAudioOutputs = await getAudioOutputs();
  
    // Compare the current list with the previous list to see if there's a change
    const hasChanged = JSON.stringify(currentAudioOutputs) !== JSON.stringify(previousAudioOutputs);
  
    if (hasChanged) {
      console.log("Audio output devices have changed.");
  
      // Show alert or other notification indicating a change in audio output devices
      // alert("Audio output devices have changed. Please check your audio settings.");
      Swal.fire({
          position: 'center',
          icon: 'error',
      ...
      previousAudioOutputs = currentAudioOutputs;
  
      // Additional actions if required
    }
  }
  
  // Initialize by setting the initial baseline
  (async function initialize() {
    previousAudioOutputs = await getAudioOutputs();
  
    // Set a periodic check for audio output changes (every 10 seconds)
    setInterval(checkForAudioOutputChanges, 10000);
  })();
...
var recordingMedia = "record-audio-plus-screen";
...
var btnStartRecording = document.querySelector('#call_flag');
...
$(document).ready(function () {
    ...
    //
    //     $('#aclip').html('Audio tag is unsupported in this browser');
    //    dataObj={
    //                'flag'      :   'start_recoding',
    //            }
    //    var final_data ={
    ...
    //    }
    //    console.log(final_data,"in func",CONFIG['domain']);
    //    $('#start_acall').html('Recording...');
    //    $('#aclip').prop('hidden',true);
    //    $.post(CONFIG['domain'] + "/ocl/rec", final_data, function (res,stat) {
...
// Define a variable to store the blob globally
let recordedBlob = null;

$("#done_btn").click(function () {

    Swal.fire({
        title: "Do you want to end interview?",
...
function getFrameRates(mediaConstraints) {
    ...
    var isScreen = false; //recordingMedia.value.toString().toLowerCase().indexOf('screen') != -1;
    if (isScreen) {
...
function getVideoResolutions(mediaConstraints) {
    ...
    if (DetectRTC.browser.name === 'Firefox') {
        mediaConstraints.video.width = 720;
        mediaConstraints.video.height = 480;
        return mediaConstraints;
    }
...
function saveToDiskOrOpenNewTab(recordRTC) {
    ...
    var downloadLink = '<a style="color:red;font-weight:500;text-decoration: underline;" href="' + URL.createObjectURL(file) + '" download="' + fileName + '">Download the File</a>';
    $("#alertMessage").empty().append('<label>' + downloadLink + '</label>');

    let formData = new FormData();

    $("#submit_btn").prop("disabled", true);

    var reg_code = $("#reg_code").html();
    var file_name = reg_code.split("_")[0];


    const videoFile = recordedBlob;
    const authKey = auth_key; 
    const libraryId =  library_id;
    const videoName = file_name; 
    
    const baseUrl = 'https://video.bunnycdn.com/library/';

    fetch(`${baseUrl}${libraryId}/videos`, {
        method: 'POST',
        headers: {
            'AccessKey': authKey,
    ...
    .then(response => {
        if (response && response.guid) {
            ...
            fetch(`${baseUrl}${libraryId}/videos/${videoGuid}`, {
                method: 'PUT',
                headers: {
                    'AccessKey': authKey,
            ...
            .then(uploadResponse => {
                if (uploadResponse.status === 200) {
                    ...
                    $.post(CONFIG['portal'] + "/api/interview-file", final_data, function (res) {
                        if (res.response == 'success') {
                            $("#upload_loadingp").prop("hidden", true);
                            $("#file_uploaded").prop("hidden", false);
                            fill_data();
                        }
                    })

                } else {
                    alert('Error in uploading to CDN')
                }
            })
            .catch(uploadError => {
    ...
    //     url: CONFIG['domain'] + '/api/get-file',
    //     data: formData,
    //     processData: false,
    //     contentType: false,
    //     cache: false,
    ...
    //         if ($("#impairment_code").val() == null || $("#action_plan").val() == null) {
    //             $("#submit_btn").prop("disabled", true);
    //         }
    //         else {
    ...
    //         $("#upload_loadingp").prop("hidden",true);
    //         var downloadLink = '<a style="color:red;font-weight:500;text-decoration: underline;" href="' + URL.createObjectURL(file) + '" download="' + fileName + '">Download the File</a>';
    //         $("#alertMessage").empty().append('<label>File upload failed. ' + downloadLink + '</label>');
...
function getFileName(fileExtension) {
    ...
    return 'VDO_' + reg_code + '.' + fileExtension;
}
...
function captureAudioPlusScreen(config) {
    ...
    if (navigator.getDisplayMedia) {
        navigator.getDisplayMedia({
            video: { width: 900, height: 675, frameRate: 15 }
        }).then(screenStream => {
        ...
        navigator.mediaDevices.getDisplayMedia({
            video: { width: 900, height: 675, frameRate: 15 }, audio: true
        }).then(screenStream => {
            let displaySurface = screenStream.getVideoTracks()[0].getSettings().displaySurface;
            console.log("displaySurface : ", displaySurface);

            if (displaySurface != "monitor") {
                alert("Please Choose the Entire Screen for Recording")
                var button = btnStartRecording;
                button.disabled = true;
                button.disableStateWaiting = true;
            ...
            navigator.mediaDevices.getUserMedia({ audio: true }).then(function (mic) {
                ...
                config.onMediaCaptured(composedStream);

                addStreamStopListener(composedStream, function () {
                    // config.onMediaStopped();
